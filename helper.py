...
def fetch_stats(selected_user, df):
    ...
    words = [word for msg in df["message"].astype(str) for word in msg.split()]
    num_media = df[df["message"].str.contains("omitted", na=False)].shape[0]
    num_urls = sum(len(extractor.find_urls(msg)) for msg in df["message"].astype(str))
...
def count_wordcloud(selected_user, df):
    ...
    def clean_text(text):
        return " ".join([word.lower() for word in str(text).split() if word not in stopwords_set])
...
def most_common_user(selected_user, df):
    ...
    stopwords_set = english_stopwords | set(stopwords_eng.split()) | bengali_stopwords
    words = [word for message in temp["message"].astype(str) for word in message.lower().split() if word not in stopwords_set]
...
def emoji_helper(selected_user, df):
    if selected_user != 'Overall':
        df = df[df['user'] == selected_user]
    emojis = [c for msg in df["message"].astype(str) for c in msg if c in emoji.EMOJI_DATA]
